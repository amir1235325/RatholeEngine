...
| **تنظیمات** (`#/settings`) | پیکربندی هاب (زبان، رفرش خودکار، کلید SSH، slug مخزن، توکن API) **+ پیکربندی سرور ایرانِ انتخاب‌شده**: دامنه/گواهی و پورت‌های `fake-port`، `sub-port`، `control-port` (از طریق action `set_config`)، با هشدار تداخل پورت پیش از ارسال. پورت‌های بقیه‌ی اجزا (internal، hub، پورت هر حامل) نمایش داده می‌شوند و به تنظیمِ مربوط به خودشان لینک می‌خورند. |
